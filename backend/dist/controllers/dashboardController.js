...
const getOverview = async (req, res) => {
    try {
        ...
        const habitsCompletedToday = habits.filter(h => h.logs.some(log => log.date >= startOfToday && log.completed)).length;
        const habitsCompletedThisWeek = habits.reduce((sum, h) => sum + h.logs.filter(log => log.date >= startOfWeek && log.completed).length, 0);
        ...
        cache_1.cache.set(cacheKey, result, cache_1.TTL.DASHBOARD_OVERVIEW);
        res.json(result);
    }
    catch (error) {
...
const getWeeklyProgress = async (req, res) => {
    try {
        ...
        const result = { days };
        cache_1.cache.set(cacheKey, result, cache_1.TTL.WEEKLY_PROGRESS);
        res.json(result);
    }
    catch (error) {
...
const getMonthlyStats = async (req, res) => {
    try {
        ...
        cache_1.cache.set(cacheKey, result, cache_1.TTL.MONTHLY_STATS);
        res.json(result);
    }
    catch (error) {
...
const getFinanceByCategory = async (req, res) => {
    try {
        const userId = req.userId;
        ...
        cache_1.cache.set(cacheKey, result, cache_1.TTL.FINANCE_CATEGORY);
        res.json(result);
    }
    catch (error) {
...
const getFinancialEvolution = async (req, res) => {
    try {
        ...
        cache_1.cache.set(cacheKey, result, cache_1.TTL.FINANCE_EVOLUTION);
        res.json(result);
    }
    catch (error) {
...
exports.getFinancialEvolution = getFinancialEvolution;
