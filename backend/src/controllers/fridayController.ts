...
export const uploadFile = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const nomesList = [];
        if (parsed.payer) nomesList.push(`  + ${parsed.payer} (pagador)`);
        if (parsed.receiver) nomesList.push(`  + ${parsed.receiver} (recebedor)`);
        if (parsed.establishment && !parsed.receiver) nomesList.push(`  + ${parsed.establishment}`);
